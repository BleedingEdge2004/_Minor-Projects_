...
const AddRecord = (e) => {
    ...
    if (expName.value.trim() !== "" && expAmount.value !== "" && expAmount.value !== "0") {
        ...
        expName.value === "" || expName.value === " " ? alert("Can't entry a Expence/Income without Name .. ") : expAmount.value === "0" ? alert("Can't entry Expence/Income with Zero Amount") : alert("Can't entry Expence/Income without Amount ");
    }
};
...
const SaveRow = (listData) => {
    let rowData = [...listData];
    localStorage.setItem(`${rowData[0]}`, JSON.stringify(rowData));
};
